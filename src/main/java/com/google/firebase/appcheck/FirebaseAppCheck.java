...
public final class FirebaseAppCheck {
  ...
  public static synchronized FirebaseAppCheck getInstance(FirebaseApp app) {
    FirebaseAppCheck.FirebaseAppCheckService service = ImplFirebaseTrampolines.getService(app, SERVICE_ID,
            FirebaseAppCheck.FirebaseAppCheckService.class);
    if (service == null) {
      service = ImplFirebaseTrampolines.addService(app, new FirebaseAppCheck.FirebaseAppCheckService(app));
    }
   ...
   * @return A {@link DecodedAppCheckToken}.
   * @throws FirebaseAppCheckException If an error occurs while getting the template.
   */
  public DecodedAppCheckToken verifyToken(@NonNull String token) throws FirebaseAppCheckException {
   ...
   *
   * @param token The App Check token to be verified.
   * @return An {@code ApiFuture} that completes with a {@link DecodedAppCheckToken} when
   *     the provided token is valid.
   */
  public ApiFuture<DecodedAppCheckToken> verifyTokenAsync(@NonNull String token) throws FirebaseAppCheckException {
    return verifyTokenOp(token).callAsync(app);
  }

  private CallableOperation<Template, FirebaseRemoteConfigException> verifyTokenOp(
          final String token) {
